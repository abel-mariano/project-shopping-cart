...
// Function that calculates the total value of the items in the shopping cart / Função que calcula o valor total dos itens no carrinho de compras
export const finalPrice = () => {
  ...
  if (totalValue === 0) {
    cartProduct.forEach((item) => {
      const productPriceValue = item.querySelector('.product__price__value').innerHTML;
      totalValue += Number(productPriceValue);
    });
    totalPrice.innerHTML = totalValue.toFixed(2);
  }
};
...
export const createProductElement = ({ id, title, thumbnail, price }) => {
  ...
  // Adiciona o produto ao carrinho de compras / Add the product to the shopping cart
  cartButton.addEventListener('click', async () => {
    const cartProducts = document.querySelector('.cart__products');
    const requestProduct = await fetchProduct(id);
    saveCartID(id);
    const addProductCard = createCartProductElement(requestProduct);
    cartProducts.appendChild(addProductCard);
    finalPrice();
  });
